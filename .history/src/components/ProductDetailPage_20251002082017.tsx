  ...
  const ProductCard = ({ product }: { product: Product }) => (
    <div className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex-shrink-0 w-64 lg:w-72">
      ...
      <div className="p-4 flex flex-col h-[160px]">
        <h3 className="text-gray-800 mb-3 line-clamp-2 text-sm lg:text-base flex-grow min-h-[48px] flex items-start">{product.name}</h3>
        <div className="flex items-center justify-between mb-4 mt-auto">
          <div className="flex items-center space-x-2">
            <span className="text-[#D42323] font-semibold">{product.price}</span>
            {product.originalPrice && (
              <span className="text-gray-400 line-through text-sm">{product.originalPrice}</span>
            )}
          </div>
        </div>
        <button className="w-full bg-[#D42323] text-white py-2 px-4 rounded-lg hover:bg-[#B91C1C] transition-colors duration-200 uppercase tracking-wide text-sm mt-auto">
          XEM CHI TIẾT
        </button>
      </div>
    </div>
